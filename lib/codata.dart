...
var txt2010 = 
'''2{20} lattice spacing of silicon                       192.015 5714 e-12     0.000 0032 e-12       m
...
parse_constants(d) {
    ...
    for (var line in lines) {
        ...
        var name = line.substring(0, 55).trim();

        var val = line.substring(55, 77).replaceAll(' ', '').replaceAll('...', '');
        val = double.parse(val);

        var uncert = line.substring(77, math.min(99, length)).replaceAll(' ', '').replaceAll('(exact)', '0');
        uncert = double.parse(uncert, (s) => 0.0);

        var units = line.substring(math.min(99, length)).trim();

        constants[name] = {"value": val, "uncertainity": uncert, "units": units};
    }
    return constants;
}

all_constants(){
    var all_consts = parse_constants(txt2002);
    all_consts.addAll(parse_constants(txt2006));
    all_consts.addAll(parse_constants(txt2010));
    all_consts.addAll(parse_constants(txt2014));

    return all_consts;
}

var Physical_constants = all_constants();
...
units(key) {
    return Physical_constants[key]["units"];
}

precision(key) {
    return Physical_constants[key]["uncertainity"] / Physical_constants[key]["value"];
}
